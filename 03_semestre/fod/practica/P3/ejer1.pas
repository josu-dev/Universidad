...
// modificacion
procedure bajaOptimizada(var f: File_Worker);
var
    bajado: Worker;
    bContinue: boolean;
    num, pos: integer;
begin
    reset(f);
    repeat
        write('Ingrese el numero de trabajador a bajar: '); readln(num);
        searhWorkerByNum(f,num,bajado);
        pos = filePos(f) -1;
        seek(f, filesize(f) - 1);
        read(f, bajado);
        seek(f, filePos(f) - 2);
...
begin
    ...
    while bApp do begin
        ...
        writeln('   8- Realizar baja de empleado');
