...
gcc -o $binary_name -O$optimization $script
if [ $? -ne 0 ]; then
    exit 1
fi
...
for msize in 0512 1024 2048 4096 ; do
    ...
    for bsize in 128 ; do
        output=./logs/out_${binary_name}_${msize}_${bsize}
        error=./logs/err_${binary_name}_${msize}_${bsize}
        sbatch -o $output -e $error -N 1 --exclusive --wrap "./$binary_name $msize $bsize"
    done
done
