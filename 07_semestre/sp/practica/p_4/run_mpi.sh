...
mpicc -o $binary_name -O$optimization $script
if [ $? -ne 0 ]; then
    exit 1
fi
...
for msize in 0512 1024 2048 ; do
    echo "Submitting batch jobs for $binary_name with msize=$msize..."
    procesors=2
    cores=$((procesors * 4))
    for nodes in 1 2 ; do
        output=./logs/out_${binary_name}_${msize}_${nodes}_${cores}
        error=./logs/err_${binary_name}_${msize}_${nodes}_${cores}
        sbatch -o $output -e $error -N $nodes --tasks-per-node=$cores --exclusive --wrap "mpirun -np $((node * cores)) ./$binary_name $msize"
    done
done
