...
public class Sistema {
	...
	public Propiedad registrarPropiedad(String nombre, String descripcion, String direccion, double precioPorNoche,
			Usuario dueño, String politicaCancelacion) {
		PoliticaCancelacion politica = null;
		switch (politicaCancelacion) {
		case "flexible":
			politica = new PoliticaFlexible();
			break;
		case "moderada":
			politica = new PoliticaModerada();
			break;
		case "estricta":
			politica = new PoliticaEstricta();
			break;
		}
		Propiedad p = new Propiedad(nombre, descripcion, direccion, precioPorNoche, dueño, politica);
	...
	public List<Reserva> obtenerReservas(Usuario usuario) {
		return this.reservas.stream().filter(r -> r.getInquilino() == usuario).toList();
	}

	public double calcularIngresos(Usuario usuario, LocalDate fechaInicio, LocalDate fechaFin) {
		return this.reservas.stream().filter(r -> r.getDueño() == usuario)
				.mapToDouble(r -> r.costo(fechaInicio, fechaFin)).sum();
	}
	
	public double calcularReembolso(Reserva reserva, LocalDate fecha) {
		return reserva.reembolso(fecha);
	}
}
