...
for msize in 512 1024 2048 4096 ; do
    ...
    for bsize in 16 32 64 128 ; do
        output=./logs/out_${name}_${msize}_${bsize}
        error=./logs/err_${name}_${msize}_${bsize}
        sbatch -o $output -e $error -N 1 --exclusive --wrap "./$name $msize $bsize"
    done
done
